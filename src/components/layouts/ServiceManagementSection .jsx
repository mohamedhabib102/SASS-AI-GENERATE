      ...
      <CustomContainer>
        ...
        <div className="flex flex-col gap-6 md:gap-8 mt-8 md:mt-12 w-full">
          {cards.map((card) => (
            <div
              key={card.id}
              className={`w-full lg:w-[85%] bg-[#EDEDFC] border border-primary rounded-3xl p-6 md:p-8 lg:p-10 flex flex-col md:flex-row items-center gap-6 md:gap-12 transition-all duration-300 hover:shadow-md hover:scale-[1.01] ${getStaggerClass(
                card.id
              )}`}
            >
              {/* Text Content */}
